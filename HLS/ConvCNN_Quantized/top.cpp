...
#pragma HLS INTERFACE s_axilite port=zpXNext bundle=CTRL_BUS
	...
	#pragma HLS RESOURCE variable=act core=XPM_MEMORY uram
...
#pragma HLS RESOURCE variable=psum_output core=XPM_MEMORY uram
	...
	//Extract our tiles for convolution
			...
			for(int tidX = 0; tidX < tileNumX; tidX++){
				...
				for(int tidIn = 0; tidIn < tileNumIn; tidIn++){
					...
					if(enableBit > Ti){
					 ...
					 // operation -- template value here sets the number of PE elements we will use
					HWConv<(int)(To)>(wgt, act, psum_output, zpX, zpW,
...
void LoadWeight(
	...
	// load weigh to the one tiled kernel spatial level
