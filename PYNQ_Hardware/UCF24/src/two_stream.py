import cv2, time, configparser
import ctypes
import numpy as np
import multiprocessing as mp
from multiprocessing import Process

from pynq import Overlay
from pynq import allocate
from LK_optical_flow.accelerator import LK_accelerator
from LK_optical_flow.utils import showarray, Feature_bank

from nnet.accelerator import CNN_accelerator
#from nnet.simpnet import simple_net, simple_net_2
from ucf24_simpnet_spatial import ucf24_simpnet_spatial
from ucf24_simpnet_temporal import ucf24_simpnet_temporal

class Two_stream(object):
    def __init__(self, config_path = './files/config.config'):
        
        config = configparser.ConfigParser()   
        config.read(config_path)
                
        overlay = Overlay(config["FPGAConfig"]["bitstream_path"])
        
        (h, w, c) = (self.in_height, self.in_width, self.in_channel) = \
                    (int(config["DataConfig"]["image_height"]),
                     int(config["DataConfig"]["image_width"]),
                     3)
        
        self.lucas_kanade_acc = LK_accelerator(config, overlay=overlay)
        self.feature_bank = Feature_bank(config)
        
        cnn_acc0 = CNN_accelerator(config, overlay.DoCompute_0, is_spatial=True)
        cnn_acc1 = CNN_accelerator(config, overlay.DoCompute_1)
        self.cnn_acc0 = cnn_acc0
        self.cnn_acc1 = cnn_acc1
        
        self.model_spatial = ucf24_simpnet_spatial('./files/params/ucf24_simpnet/spatial_model.pickle', config, cnn_acc0)

        self.model_temporal = ucf24_simpnet_temporal('./files/params/ucf24_simpnet/temperal_model.pickle', config, cnn_acc1)
        
        self.output = mp.Array(ctypes.c_float, 24*2)
        
        self.delay_list = []
                
    def spatial_job(self, input_frame, output): 
        start = time.time()       
        out = self.model_spatial(input_frame)        
        self.output[:24] = out
        end = time.time()
        
        # print(f"SCNN elapse: {(end-start)} s")
            
    def temporal_job(self, input_frame, old_frame, output):
        start = time.time()
        temp_frame = self.feature_bank.get_np_arr()        
        out = self.model_temporal(temp_frame[:,:,:20]) 
        self.output[24:] = out
        end = time.time()
        
        # print(f"TCNN elapse: {(end-start)} s")
            
    def __call__(self, input_frame, old_frame) -> np.ndarray :
        assert isinstance(input_frame, np.ndarray) and input_frame.dtype == np.uint8
        assert (self.in_height, self.in_width, self.in_channel) == input_frame.shape
        
        start = time.time()
        
        ps1 = Process(target=self.temporal_job, args=(input_frame, old_frame, 0)) #reads from feature bank        
        ps1.start()
        ps0 = Process(target=self.spatial_job, args=(input_frame, 0))
        ps0.start()
        
        ps1.join()
        ps0.join()
        
        end = time.time()
        
        # print(f"CNN process life elapse {end-start} s")
        
        #Ensure that the scalers are the same as in the Fusion training framework
        return np.array(self.output[:24])*0.5+np.array(self.output[24:])*0.5 #spatial:(0~24) temporal:(102~202)
    
    
