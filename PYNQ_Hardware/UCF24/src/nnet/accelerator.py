...
class LK_accelerator(object):
    ...
    def __init__(self, row=256, col=256, mode='signed', overlay=None):
        ...
        # create share memory
        self.inp1_arr = allocate(shape=(row,col), dtype=np.uint8)
        self.inp2_arr = allocate(shape=(row,col), dtype=np.uint8)
        self.vx_arr = allocate(shape=(row,col), dtype=np.int8)
        self.vy_arr = allocate(shape=(row,col), dtype=np.int8)
...
#             print("Initialize configuration...") 
        ...
        self.precision = int(config["PEConfig"]["precision"])     #8
        self.data_width = int(config["PEConfig"]["data_width"])   #128
...
#             self.img_channel = 32
        ...
        self.WORD_LENGTH = int(ceil(self.data_width/self.precision)) #128/8
...
#             self.img_channel += self.Ti - (self.img_channel % self.Ti)
        ...
        self.buffer_depth = int(ceil((self.img_channel*self.img_height*self.img_width)/self.WORD_LENGTH)) #Number of 128bit values
...
#         print("Initialize configuration...: done") 
    ...
    def mem_alloc(self, out_channel, in_channel, in_height, in_width, ker):
        ...
        fm_buff = allocate(shape=(fm_depth, self.WORD_LENGTH), dtype=np.uint8)
        wgt_buff = allocate(shape=(wgt_depth, self.WORD_LENGTH), dtype=np.uint8)
    ...
    def init_weight_old(self, param_path):
...
#             print("l_idx = ", l_idx, ", l.type = ", l.type)
    ...
    def init_weight(self, param_path):
        stat_dict = pickle.load(open(param_path, "rb"))
        param_list = list(stat_dict.values()) #eliminates the first layer of keys module.features.Conv2d0
        last_layer = None
        l_idx = 0
        for l in self.layers:
#             print("l_idx = ", l_idx, ", l.type = ", l.type)
            if l.type not in ['conv', 'linear']:
                continue
            if l.quantize is True:
                l.weight_data = param_list[l_idx]['weight'].astype(np.uint8)
                l.multiplier = param_list[l_idx]['scale']
                l.zp_x = param_list[l_idx]['zero_point']
                l.zp_w = 0 #param_list[l_idx]['w_zeropoint']
                l.zp_x_next = 0 #param_list[l_idx]['xnext_zeropoint']
                if(last_layer != None):
                    last_layer.zp_x_next = param_list[l_idx]['zero_point']
                    
            else:
                l.weight_data = param_list[l_idx]['weight']
                l.multiplier = param_list[l_idx]['scale']
                l.zp_x = param_list[l_idx]['zero_point']
    ...
    """ It loads a state_dict file and extracts weights and loads them to the layers of the accelerator"""        
    def load_model_weights(self, param_path):
        stat_dict = pickle.load(open(param_path, "rb"))
        param_list = list(stat_dict.values()) #eliminates the first layer of keys module.features.Conv2d0
        last_layer = None
        l_idx = 0
...
#             print("l_idx = ", l_idx, ", l.type = ", l.type)
            ...
            if l.quantize is True:
                l.weight_data = param_list[l_idx]['weight'].astype(np.uint8)
                l.multiplier = param_list[l_idx]['scale']
                l.zp_x = param_list[l_idx]['zeropoint']
                l.zp_w = 0 #param_list[l_idx]['w_zeropoint']
                l.zp_x_next = 0 #param_list[l_idx]['xnext_zeropoint']
                if(last_layer != None):
                    last_layer.zp_x_next = param_list[l_idx]['zeropoint']
    ...
    """ It loads a state_dict file and extracts weights and loads them to the layers of the accelerator"""        
    def load_model_weights2(self, param_path):
        stat_dict = pickle.load(open(param_path, "rb"))
        param_list = list(stat_dict.values()) #eliminates the first layer of keys module.features.Conv2d0
        last_layer = None
        l_idx = 0
...
#             print("l_idx = ", l_idx, ", l.type = ", l.type)
            ...
            if l.quantize is True:
                l.weight_data = param_list[l_idx]['weight'].astype(np.uint8)
                l.multiplier = param_list[l_idx]['scale'][0]
                l.zp_x = param_list[l_idx]['zero_point'][0]
                l.zp_w = 0 #param_list[l_idx]['w_zeropoint']
                l.zp_x_next = 0 #param_list[l_idx]['xnext_zeropoint']
                if(last_layer != None):
                    last_layer.zp_x_next = param_list[l_idx]['zero_point']
    ...
    """ It loads a state_dict file and extracts weights and loads them to the layers of the accelerator"""        
    def load_model_weightsTorch(self, param_path):
        stat_dict = pickle.load(open(param_path, "rb"))
        param_list = list(stat_dict.values()) #eliminates the first layer of keys module.features.Conv2d0
        last_layer = None
        l_idx = 0
...
#             print("l_idx = ", l_idx, ", l.type = ", l.type)
            if l.type not in ['conv', 'linear']:
                continue
            if l.quantize is True:
                l.weight_data = param_list[l_idx]['weight'].int_repr().numpy().astype(np.uint8)
                l.multiplier = param_list[l_idx]['scale'].numpy()
                l.zp_x = param_list[l_idx]['zeropoint'].numpy()
                l.zp_w = 0 #param_list[l_idx]['w_zeropoint']
                l.zp_x_next = 0 #param_list[l_idx]['xnext_zeropoint']
                if(last_layer != None):
                    last_layer.zp_x_next = param_list[l_idx]['zeropoint'].numpy()
                    
            else:
                l.weight_data = param_list[l_idx]['weight'].int_repr().numpy()
                l.multiplier = param_list[l_idx]['scale'].numpy()
                l.zp_x = param_list[l_idx]['zeropoint'].numpy()
    ...
    raw_image: 
        ...
        3. 
    """
    def _convert_raw_image_to_buffer__(self, raw_image):
        ...
        if img_channel > self.Ti and (img_channel % self.Ti != 0):
            zero_padding = np.zeros((imgH, imgW, self.Ti-(img_channel % self.Ti)), dtype = np.uint8)
            raw_image = np.concatenate((raw_image, zero_padding), axis = 2)
    ...
    """
    Convert pytorch IFM to allocate input
    Input: 
        1. ifm: pytorch tensor(batch, in channel, height, width)
    Output:
        1. ifm_cma: allocate cma(L, WORD_LENGTH), L = (in_channel/Ti)*height*width*(Ti/WORD_LENGTH)
    """
    # def convertIFM(ifm, Ti, depth, WORD_LENGTH):
    def _convert_raw_image_to_buffer_(self, raw_image):
        inH = raw_image.shape[0]; inW = raw_image.shape[1]
        in_channel = raw_image.shape[2]
        tmp = np.transpose(raw_image[0,:,:,:].reshape((int(in_channel/self.Ti), self.Ti, inH, inW)), (0,2,3,1))\
        .reshape((int(in_channel/self.Ti), inH, inW, int((self.Ti/self.WORD_LENGTH)), self.WORD_LENGTH)).reshape(-1,self.WORD_LENGTH)

        tmp = np.append(tmp, np.zeros((depth - tmp.shape[0], self.WORD_LENGTH),np.uint8), axis=0)

        np.copyto(self.input_buff, tmp)
    ...
    Input:
        1. wgt: pytorch tensor(out channel, in channel, kernel_height, kernel_width)
    Output:
        1. wgt_cma: allocate(Depth, WORD_LENGTH), 
    ...
    """
    Convert pytorch WGT to Xilinx allocation input
    Input:
        1. wgt: pytorch tensor(out channel, in channel, kernel_height, kernel_width)
    Output:
    ...
    """
    def _convert_weight_to_buffer_(self, layer):
        ...
        print("Weight data shape is:", layer.weight_shape)

        wgt_tmp = wgt.reshape((int(out_channel/self.To), self.To, int(in_channel/self.Ti), self.Ti, kerH, kerW))
        wgt_tmp = np.transpose(wgt_tmp, (0,2,1,4,5,3))
        wgt_tmp = wgt_tmp.reshape((int(out_channel/self.To), int(in_channel/self.Ti), self.To, kerH, kerW,int(self.Ti/self.WORD_LENGTH),self.WORD_LENGTH))
        wgt_tmp = wgt_tmp.reshape(-1,self.WORD_LENGTH)

        wgt_tmp = np.append(wgt_tmp, np.zeros((depth - wgt_tmp.shape[0], self.WORD_LENGTH),np.uint8), axis=0)
        np.copyto(layer.wgt_buff, wgt_tmp)
