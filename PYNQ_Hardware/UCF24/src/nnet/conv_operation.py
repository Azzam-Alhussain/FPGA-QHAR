...
Output:
    1. ifm_cma: allocate(L, WORD_LENGTH), L = (in_channel/Ti)*height*width*(Ti/WORD_LENGTH)
"""
def convertIFM(ifm, Ti, depth, WORD_LENGTH):
    ...
    buff = allocate(shape=(depth, WORD_LENGTH), dtype=np.uint8)
    np.copyto(buff, tmp)
    return buff
...
Output:
    1. wgt_cma: allocate(L, WORD_LENGTH), 
