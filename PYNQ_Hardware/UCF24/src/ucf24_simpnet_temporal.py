...
def make_layers_new(config, in_channel=20, accelerator=None):
...
def make_layers(config, in_channel=20, accelerator=None):
    assert config is not None    
    in_height = int(config["DataConfig"]["image_height"])
    in_width = int(config["DataConfig"]["image_width"])
    in_channel = in_channel
    ...
    layers = []
    #Conv(output channel, input channel, input height, input width, kerSize, stride)
    layers += [fpga_nn.Conv2DPool(64, in_channel, in_height, in_width, ker = 3, poolWin = 2, accelerator=acc)]
...
class UCF24SimpNet_Temporal(CNN_accelerator):
    def __init__(self, config, layers, params_path = None):
        super(UCF24SimpNet_Temporal, self).__init__(config)
        self.layers = layers
...
def ucf24_simpnet_temporal(model_path, config, accelerator):
    layers = make_layers(config, in_channel=20, accelerator=accelerator)
    params_path = model_path
    model = UCF24SimpNet_Temporal(config, layers, params_path = params_path)
    return model        
...
if __name__=='__main__':
    ...
    ucf24_spatial_model = ucf24_simpnet_temporal(model_path, config, cnn_acc0)
