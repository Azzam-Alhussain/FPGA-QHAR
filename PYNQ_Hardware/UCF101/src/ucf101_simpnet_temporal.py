...
class UCF101SimpNet_Temporal(CNN_accelerator):
    def __init__(self, config, layers, params_path = None):
        super(UCF101SimpNet_Temporal, self).__init__(config)
        self.layers = layers
...
def ucf101_simpnet_temporal(model_path, config, accelerator):
    layers = make_layers(config, in_channel=20, accelerator=accelerator)
    params_path = model_path
    model = UCF101SimpNet_Temporal(config, layers, params_path = params_path)
    return model        
...
#         scale=0.3065591626533275, zeropoint=6)
#
...
if __name__=='__main__':
    ...
    ucf101_spatial_model = ucf101_simpnet_temporal(model_path, config, cnn_acc0)
