...
def make_layers_new(config, in_channel=3, accelerator=None):
    ...
    layers += [fpga_nn.Conv2(32, in_channel, in_height, in_width, ker = 3, accelerator=acc)]

    layers += [fpga_nn.Conv2D(64, 32, int(in_height), int(in_width), ker = 3, accelerator=acc)]

    layers += [fpga_nn.Conv2DPool(64, 64, int(in_height), int(in_width), ker = 3, poolWin = 4, accelerator=acc)]
    
    layers += [fpga_nn.Conv2DPool(64, 64, int(in_height/4), int(in_width/4), ker = 3, poolWin = 4, accelerator=acc)]

    layers += [fpga_nn.Conv2DPool(64, 64, int(in_height/16), int(in_width/16), ker = 3, poolWin = 16, accelerator=acc)]

#     layers += [fpga_nn.Conv2DPool(64, 64, int(in_height/32), int(in_width/32), ker = 3, poolWin = 2, accelerator=acc)]

    # conv output size = (8,8,512)
    layers += [fpga_nn.Flatten(int(in_height/256),int(in_width/256), 64)]
    layers += [fpga_nn.Linear(101,int(in_height/256)*int(in_width/256)*64)]
    #layers += [fpga_nn.Linear(101,64, quantize = False)]
...
class UCF101SimpNetSpatial(CNN_accelerator):
    def __init__(self, config, layers, params_path = None):
        super(UCF101SimpNetSpatial, self).__init__(config, is_spatial=True)
        self.layers = layers
...
def ucf101_simpnet_spatial(model_path, config, accelerator):
    layers = make_layers(config, in_channel=3, accelerator=accelerator)
    params_path = model_path
    model = UCF101SimpNetSpatial(config, layers, params_path = params_path)
    return model        
...
if __name__=='__main__':
    ...
    ucf101_spatial_model = ucf101_simpnet_spatial(model_path, config, cnn_acc0)
