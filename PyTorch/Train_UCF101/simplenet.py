...
class SimpleNet(nn.Module):
    def __init__(
        ...
        print("self.cfg[self.networks[network_idx]][-1][0]::", self.cfg[self.networks[network_idx]][-1][0])
        #self.classifier = nn.Linear(round(self.cfg[self.networks[network_idx]][-1][0] * scale), num_classes, bias=False)
        self.classifier = nn.Sequential(
           nn.Flatten(),
            # nn.Dropout(0.5),
            nn.Linear(4096, 128, bias=False), #1024 is input size and network architecture dependent
            # nn.ReLU(True),
            nn.Dropout(0.2),
            # nn.Linear(4096, 4096),
            # nn.ReLU(True),
    ...
    def forward(self, x):
        if(self.qat):
            x = self.quant(x)
        #print("Shape x b4 ft", x.shape) 
        out = self.features(x)
        # print("Shape out b4 pl with:", out.size()[2:],"out:", out.shape) 
        # out = F.max_pool2d(out, kernel_size=out.size()[2:])
        out = F.max_pool2d(out, kernel_size=(2, 2))
        # print("Shape out b4 dr", out.shape) 
        out = F.dropout2d(out, self.last_dropout_rate, training=self.training)  
        # print("Shape b4 vw", out.shape)      
        # out = out.view(out.size(0), -1) 
        # out = torch.flatten(out, 1) #Flatten leave batch size alone
        #out = out.unsqueeze(1)  
        #out = torch.transpose(out, 1)     #added to transpose
        # print("Shape b4 tr", out.shape)
        #out = out.transpose(0, -1)
        # print("Shape b4 cl", out.shape)
    ...
    def _make_layers(self, scale: float):
        ...
        for idx, (layer, stride, defaul_dropout_rate, *layer_type) in enumerate(
            ...
            if layer == "p":
                layers += [
                    (f"MaxPool2d{idx}", nn.MaxPool2d(kernel_size=(2, 2))),
                    (f"Dropout2d{idx}", nn.Dropout2d(p=custom_dropout, inplace=True)),
                ]
            else:
                filters = round(layer * scale)
                if custom_dropout is None:
                    layers += [
                        (f"Conv2d{idx}", nn.Conv2d(input_channel, filters, kernel_size=kernel_size, stride=stride, padding=padding, bias=False)),
                        (f"BatchNorm2d{idx}", nn.BatchNorm2d(filters, eps=1e-05, momentum=0.05, affine=True)),
                        (f"Relu{idx}", nn.ReLU(inplace=True)),
                    ]
                else:
                    layers += [
                        (f"Conv2d{idx}", nn.Conv2d(input_channel, filters, kernel_size=kernel_size, stride=stride, padding=padding, bias=False)),
                        (f"BatchNorm2d{idx}", nn.BatchNorm2d(filters, eps=1e-05, momentum=0.05, affine=True)),
                        (f"Relu{idx}", nn.ReLU(inplace=True)),
                        (f"Dropout2d{idx}", nn.Dropout2d(p=custom_dropout, inplace=False)),
                    ]
...
def _gen_simplenet(
    model_variant: str = "simplenet_ucf",
    num_classes: int = 1000,
    in_chans: int = 3,
    scale: float = 1.0,
    network_idx: int = 0,
...
if __name__ == "__main__":
    model = simplenet(network_idx=0, num_classes=1000)
